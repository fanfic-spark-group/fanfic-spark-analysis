...
- **Milestone 3 baseline hits the majority-class floor.** `RandomForestClassifier(numTrees=20, maxDepth=5)` on the §6f numeric pipeline (`text_len`, `perplexity_score`, engineered `text_len_log10`, `len_per_perplexity`; median-imputed + min-max scaled) reaches test accuracy ≈ the §6e `'other'` share (~0.842); every §7c sample across train/validation/test is predicted `'other'`. Train ≈ validation ≈ test on all four metrics (gap ≈ 0) — severe underfit, not overfit.
- **Milestone 3 capacity sweep: not the bottleneck.** §8b's deeper (T=20, D=15) and wider (T=100, D=5) RF variants produce metrics identical to the baseline and never open a train-test gap; the deeper variant costs several× the baseline's fit time for zero metric improvement. Even with the §6f engineered features and scaling, the numeric feature space cannot separate the 51 fandom-genre classes regardless of model capacity. The bottleneck is feature *information*, not classifier capacity.
