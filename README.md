...
Full data exploration is in [`notebook/analysis.ipynb`](notebook/analysis.ipynb), covering SparkSession initialization, dataset loading from Lustre and partition inspection, §3 data exploration (`describe`, `groupBy`, distinct counts, missing/duplicate analysis), the six §4 plots with their analysis cells, and the §5 written preprocessing plan for Milestone 3.
